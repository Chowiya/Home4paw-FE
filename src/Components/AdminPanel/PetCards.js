...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/approving/${props.pet._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`${process.env.REACT_APP_URL}/form/delete/many/${props.pet._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${user.token}`
        }
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/delete/${props.pet._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${user.token}`
        }
